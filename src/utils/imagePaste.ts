...
function getClipboardCommands() {
  ...
  // Validate against shell injection characters:
  let tmpDirEnv = process.env.CLAUDE_CODE_TMPDIR
  if (tmpDirEnv && /[";&|`<>$]/.test(tmpDirEnv)) {
    tmpDirEnv = undefined // Ignore potentially malicious path
  }
  let winTempEnv = process.env.TEMP
  if (winTempEnv && /[";&|`<>$]/.test(winTempEnv)) {
    winTempEnv = undefined
  }
  const baseTmpDir =
    tmpDirEnv ||
    (platform === 'win32' ? winTempEnv || 'C:\\Temp' : '/tmp')
  ...
  > = {
    ...
    win32: {
      ...
      saveImage: `powershell -NoProfile -Command "$img = Get-Clipboard -Format Image; if ($img) { $img.Save('${screenshotPath.replace(/\\/g, '\\\\').replace(/'/g, "''")}', [System.Drawing.Imaging.ImageFormat]::Png) }"`,
...
export async function getImageFromClipboard(): Promise<ImageWithDimensions | null> {
  ...
  try {
    ...
    getFsImplementation().unlink(screenshotPath).catch(() => {})
