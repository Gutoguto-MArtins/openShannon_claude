...
export function buildLinuxClipboardSaveCommand(): string {
  return LINUX_CLIPBOARD_IMAGE_MIME_TYPES.flatMap(mimeType => [
    `xclip -selection clipboard -t ${mimeType} -o > "$CLAUDE_CLIPBOARD_TMP_PATH" 2>/dev/null`,
    `wl-paste --type ${mimeType} > "$CLAUDE_CLIPBOARD_TMP_PATH" 2>/dev/null`,
  ]).join(' || ')
}
...
function getClipboardCommands() {
  ...
  > = {
    darwin: {
      ...
      saveImage: `osascript -e 'set png_data to (the clipboard as «class PNGf»)' -e 'set tmp_path to system attribute "CLAUDE_CLIPBOARD_TMP_PATH"' -e 'set fp to open for access POSIX file tmp_path with write permission' -e 'write png_data to fp' -e 'close access fp'`,
    ...
    win32: {
      ...
      saveImage: `powershell -NoProfile -Command "$img = Get-Clipboard -Format Image; if ($img) { $img.Save($env:CLAUDE_CLIPBOARD_TMP_PATH, [System.Drawing.Imaging.ImageFormat]::Png) }"`,
...
export async function getImageFromClipboard(): Promise<ImageWithDimensions | null> {
  ...
  try {
    ...
    const saveResult = await execa(commands.saveImage, {
      shell: true,
      reject: false,
      env: { ...process.env, CLAUDE_CLIPBOARD_TMP_PATH: screenshotPath },
    })
    ...
    void getFsImplementation().unlink(screenshotPath).catch(() => {})
