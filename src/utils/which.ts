...
async function whichNodeAsync(command: string): Promise<string | null> {
  if (process.platform === 'win32') {
    ...
    const result = await execa('where.exe', [command], { shell: false, stderr: 'ignore', reject: false })
  ...
  const result = await execa('which', [command], { shell: false, stderr: 'ignore', reject: false })
  if (result.exitCode !== 0 || !result.stdout) {
    return null
  }
